...
class Solution(object):
    def searchRange(self, nums, target):
        ...
        """
        n = len(nums)
        first = self.findFirst(nums,target,n)
        if first==-1 or first == n-1:
            return [first,first]
        least = self.findLeast(nums,target,n, first)
        return [first,least]
        
        
    def findFirst(self,nums,t,n):
        s,e = 0,n-1
    ...
    def findLeast(self,nums,t,n,f):
        ...
        while(s<=e):
            ...
            mid = nums[m]
            if mid == t:
                if m<n-1 and nums[m+1]!= t or m==n-1:
                    return m
                else:
                    s = m+1
            elif mid>t:
